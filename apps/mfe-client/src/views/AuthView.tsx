...
import { loginSchema, registerClientSchema, LoginFormData, RegisterClientFormData } from '../lib/schemas/auth.schema';
...
export const AuthView = ({ onAuth, lang, setLang, darkMode, toggleTheme, onBack }: any) => {
  ...
  const { register, handleSubmit, formState: { errors }, clearErrors } = useForm<RegisterClientFormData>({
    resolver: zodResolver(isSignUp ? registerClientSchema : loginSchema),
  ...
  const onSubmit = async (data: RegisterClientFormData) => {
    setLoading(true);
    setAuthError('');
    const success = await onAuth(isSignUp, data);
    if (!success) {
      setAuthError("Identifiants incorrects ou erreur lors de l'inscription.");
    }
    setLoading(false);
  };
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-8 md:p-16">
            <div className="max-w-md mx-auto h-full flex flex-col justify-center">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                {isSignUp && (
                  <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <AuthInput label="Prénom" icon={<User />} placeholder="Jean" {...register("firstname")} />
                        {errors.firstname && <span className="text-xs text-red-500 font-bold ml-1 mt-1 block">{errors.firstname.message}</span>}
                      </div>
                      <div>
                        <AuthInput label="Nom" icon={<User />} placeholder="Dupont" {...register("lastname")} />
                        {errors.lastname && <span className="text-xs text-red-500 font-bold ml-1 mt-1 block">{errors.lastname.message}</span>}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div>
                  <AuthInput label="Email" icon={<Mail />} type="email" placeholder="client@email.com" {...register("email")} />
                  {errors.email && <span className="text-xs text-red-500 font-bold ml-1 mt-1 block">{errors.email.message}</span>}
                </div>
                <div>
                  <AuthInput label="Mot de passe" icon={<Lock />} type="password" placeholder="••••••••" {...register("password")} />
                  {errors.password && <span className="text-xs text-red-500 font-bold ml-1 mt-1 block">{errors.password.message}</span>}
                </div>
                ...
                {authError && (
                  <div className="p-3 bg-red-50 dark:bg-red-900/20 border-2 border-red-100 dark:border-red-900/30 text-red-600 dark:text-red-400 text-xs font-bold rounded-xl flex items-center gap-2">
                    <div className="size-1.5 bg-red-600 rounded-full animate-pulse" /> {authError}
                  </div>
                )}
                ...
                >
                  {loading ? <Loader2 className="animate-spin size-5" /> : (
                    <>
                      {isSignUp ? "Confirmer l'inscription" : "Me connecter"}
                      <ArrowRight size={18} />
                    </>
                  )}
                </button>
