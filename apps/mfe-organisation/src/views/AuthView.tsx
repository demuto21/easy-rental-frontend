...
import { loginSchema, registerOrgSchema, RegisterOrgFormData } from '../lib/schemas/auth.schema';
...
export const AuthView = ({ onAuth, lang, setLang, darkMode, toggleTheme, t }: any) => {
  ...
  const { register, handleSubmit, formState: { errors }, clearErrors } = useForm<RegisterOrgFormData>({
    resolver: zodResolver(isSignUp ? registerOrgSchema : loginSchema),
    defaultValues: { firstname: '', lastname: '', email: '', password: '', name: '' }
  });
                ...
                {isSignUp && (
                  <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <AuthInput label={t.auth.firstname} icon={<User />} placeholder="Jean" {...register("firstname")} />
                      </div>
                      <div>
                        <AuthInput label={t.auth.lastname} icon={<User />} placeholder="Dupont" {...register("lastname")} />
                      </div>
                    </div>
                    <div>
                      <AuthInput label={t.auth.orgName} icon={<Building />} placeholder="Global Rent Ltd" {...register("name")} />
                      {errors.name && <span className="text-xs text-red-500 font-bold ml-1 mt-1 block">{errors.name.message}</span>}
                    </div>
                  </div>
                )}

                <div>
                  <AuthInput label={t.auth.email} icon={<Mail />} type="email" placeholder="manager@rental.com" {...register("email")} />
                  {errors.email && <span className="text-xs text-red-500 font-bold ml-1 mt-1 block">{errors.email.message}</span>}
                </div>
                <div>
                  <AuthInput label={t.auth.password} icon={<Lock />} type="password" placeholder="••••••••" {...register("password")} />
