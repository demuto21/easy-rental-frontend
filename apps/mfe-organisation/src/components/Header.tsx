...
export const Header = ({ orgData, lang, setLang, darkMode, toggleTheme, setSidebarOpen, onInstall, hasPrompt, t }: any) => {
  const router = useRouter();
  const pathname = usePathname();
  const title = pathname === '/' ? t?.sidebar?.dashboard : pathname.replace('/', '').toUpperCase();
  ...
  return (
    <header className="h-20 px-4 md:px-10 flex items-center justify-between shrink-0 border-b-4 border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-[#0f1323]/80 backdrop-blur-lg sticky top-0 z-[50]">
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <button onClick={() => router.push('/notifications')} className="p-2.5 text-slate-400 hover:text-[#0528d6] hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-full transition-all relative group">
        ...
        <div onClick={() => router.push('/profile')} className="flex items-center gap-3 ml-2 pl-4 border-l border-slate-200 dark:border-slate-800 cursor-pointer group">
