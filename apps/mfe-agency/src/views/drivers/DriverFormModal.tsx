...
import { driverSchema, DriverFormData } from '../../lib/schemas/driver.schema';
...
export const DriverFormModal = ({ editingDriver, onSubmit, onClose, modalLoading, error, t }: any) => {
  ...
  const handleLocalSubmit = (data: DriverFormData) => {
    const formData = new FormData();
    Object.entries(data).forEach(([k, v]) => formData.append(k, (v ?? '').toString()));
        ...
        <form onSubmit={handleSubmit(handleLocalSubmit)}
          ...
          <div className="p-6 md:p-10 overflow-y-auto custom-scrollbar space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              <Input label={t.auth.firstname} error={errors.firstname?.message} required icon={<User size={14} />} {...register("firstname")} />
              <Input label={t.auth.lastname} error={errors.lastname?.message} required icon={<User size={14} />} {...register("lastname")} />
              <Input label={t.agencies.modal.phone} type="tel" error={errors.tel?.message} required icon={<Phone size={14} />} {...register("tel")} />
              <div className="grid grid-cols-2 gap-4">
                <Input label={t.driverForm.age} type="number" error={errors.age?.message} required {...register("age")} />
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase italic ml-1 tracking-widest">{t.driverForm.gender}</label>
                  <select {...register("gender")} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-xl font-bold text-xs outline-none focus:border-[#0528d6] dark:text-white transition-all">
...
const Input = React.forwardRef<HTMLInputElement, any>(({ label, type = "text", required = false, icon, error, ...props }, ref) => (
